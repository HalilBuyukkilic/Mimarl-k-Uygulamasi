                    ...
                    <div class="card p-5 mt-3" style="background-color: white">
                        <div class="d-flex justify-content-between">
                            <span style="font-size: 22px;">
                                <i class="fas fa-newspaper mr-1"></i> Haberler Listesi
                            </span>
                            <span>
                                <div class="btn-group dropleft" style="margin-top: 5px">
                                    ...
                                    <div class="dropdown-menu">
                                        <a href="{{ route('admin.deleted.news') }}"
                                            class="ml-1 back_textColor backbg_fourthly p-1">
                                            <i class="far fa-eye-slash"></i> Silinen Haberler
                                        </a>
                                    </div>
                                </div>

                                <a href="{{ route('admin.news_create') }}"
...
<script>
    $(document).ready(function() {
        ...
        let newsDT = $("#news").DataTable({
            ...
            "serverSide": true,
            "ajax": {
                url: "{{ url('admin/haberler_veritablosu') }}",
                type: 'GET'
            },
            columns: [{
                    data: 'id',
                ...
                },
                {
                    ...
                    className: 'text-center',
                    render: function(data) {
                        var url = '{{ route('admin.news_edit', ':id') }}';
                        url = url.replace(':id', data.id);
                        var url_ui = '{{ route('ui.news.page', ':slug') }}';
                        url_ui = url_ui.replace(':slug', data.slug);
                        ...
                        if (data.status == 1) {
                            if (data.showing_modal) {
                                html +=
                                    '<a class="btn backbg_secondary  back_textColor"  href="' +
                                    url_ui +
